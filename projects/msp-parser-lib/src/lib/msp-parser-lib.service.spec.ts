...
describe('gwMspService test', () => {
  describe('when I call gwMspService.convert', () => {
    let service: MspParserLibService;
    ...
    it('should return one spectra for the given test file with 1 spectra in it', () => {
      const data =
      ...
      const result = service.convertFromData(data, (res) => {
        expect(res.names[0]).toEqual('glutamate');
        expect(res.meta.length).toEqual(4);
        expect(res.accurate).toBeFalsy();
        expect(res.spectrum.split(' ').length).toEqual(151);
      });

      expect(result).toBeTruthy();
    });

    it('should return one spectra for the given test file with 1 spectra in it, which contains double values as spectra object', () => {
      const data =
    ...
    it('should return one spectra for given test file with 1 spectra in it', () => {
      const data =
        'Name: Carnosic Acid\n' +
      ...
      const result = service.convertFromData(data, (res) => {
        ...
        expect(res.spectrum.split(' ').length).toEqual(11);
      });

      expect(result).toBeTruthy();
    });

    it('should return one spectra for the given test file with 1 spectra in it, which contains double values as spectra object and is an accurate mass spectra', () => {
      const data =
    ...
    it('should return one spectra for the given test file with 1 spectra in it, which contains double values as spectra object and is an accurate mass spectra and have external ids', () => {
      const data =
    ...
    it('should properly parse the spectrum', () => {
      const data =
        'Name: C4\n' +
        'RI: 262214\n' +
        'BinId: 21\n' +
      ...
      const result = service.convertFromData(data, (res) =>  {
        ...
        expect(res.accurate).toBeFalsy();
        expect(res.spectrum.split(' ').length).toEqual(44);
      });
